...
void dread4(int fd, uintptr_t addr, void* buf, int sz) {

  char string_buf[16];
  bzero(string_buf, sizeof(string_buf));

  ioctl(666, 0xDEADBEEF, addr, &string_buf);
  ...
  unsigned char address_buf[sizeof(struct in_addr)];
  //printf("got addr %s\n", string_buf);
  err = inet_pton(AF_INET, string_buf, address_buf);
  if (err <= 0) {
    perror("inet_pton");
    exit(1);
  }

  memcpy(buf, address_buf, sz);
...
void dread(int fd, uintptr_t addr, void* buf, int sz) {
  ...
  for (int i = 0; i < sz; i += 4) {
    dread4(fd, addr + i, ((char*)buf) + i, MIN(4, sz - i));
  }
}
...
int main(int argc, char** argv) {
    ...
    (void) dtrace_setopt(handle, "dynvarsize", "1024");
    (void) dtrace_setopt(handle, "destructive", "yes");


    dtrace_prog_t *prog = dtrace_program_strcompile(handle, "syscall::ioctl:entry / arg1 == 0xDEADBEEF && execname == \"global_ps2\" / { this->v = inet_ntoa((in_addr_t*)arg2); copyoutstr(this->v, (uintptr_t)arg3, 16); }",
